...
velocity = 10. * u.centimeter / u.year
...
Model = GEO.Model(
    ...
    maxCoord=(100. * u.kilometer, 9. * u.kilometer),
    gravity=(0.0, -9.81 * u.meter / u.second**2))
Model.outputDir = "240x30-serial_hi_nofilter_sw_b20_Thrust_Sheet-v0.1_t"
...
plasticLayer.plasticity = GEO.DruckerPrager(
    ...
    cohesionAfterSoftening=2 * u.mpascal,
...
# In[14]:
Model.stretching = Model.add_swarm_field( "stretching",dataType="double", count=2)
Model.orientation = Model.add_swarm_field( "orientation",dataType="double", count=1)
Model.stretching.data[:,0] = 1.0
Model.stretching.data[:,1] = 0.0
Model.orientation.data[:] = 0.0
Model.init_model()
...
def plotFigures(fnSize=2, figSize=(1600, 300)):
    ...
    Model.plot.velocityField(
        figsize=(1600, 300)).save(Model.outputDir + '/Velocity_ ' + str(Model.time) + '.png')

    Model.plot.material(
        figsize=figSize,
        fn_size=fnSize,
        fn_mask=Model.materialField > stickyAirLayer.index,
        valueRange=[stickyAirLayer.index,rigidBase.index]).save(
...
def ErosionAndSedimentation(airIndex, sedimentIndex):
    ...
    Model.plasticStrain.data[sedimented_mask] = 0
    ...
    Model.plasticStrain.data[eroded_mask] = 0
    return
...
plotFigures(fnSize=4)
Model.run_for(.005 * u.megayears,dt=.005 * u.megayears)
plotFigures(fnSize=4)
Model.meshStretching=Model.add_mesh_field(name="meshStretching",nodeDofCount=2)
Model.swarmVgrad=Model.velocityField.fn_gradient.evaluate(Model.swarm)
swarmVgrad = Model.velocityField.fn_gradient.evaluate(Model.swarm)
Model.orientation.data[:,0] = np.where( np.abs(Model.swarmVgrad[:,0]) > 1.1, -180 * np.arctan2(Model.stretching.data[:,1] / math.pi, Model.stretching.data[:,0]) / math.pi, 0.0)
projectStretching=GEO.underworld.utils.MeshVariable_Projection( meshStretching, stretching, type=0 )
...
#figMeshStretching.append( glucifer.objects.VectorArrows(Model.mesh, meshStretching, scaling=0.1, arrowHead=0.00001, resolutionI=100, resolutionJ=25)) 
 
figMeshStretching.append( glucifer.objects.Points(Model.swarm, orientation , pointSize=5.0, colours="#448800, #666666:0.0, #0099FF") )